...
\TODO{SM est-ce qu'on parle aussi des pbs de ``time drifting'' ? \\
  DB c'est couvert, après si tu veux mettre le mot clé ça me va\ldots
   sauf que je sais pas de quoi tu veux parler: si c'est le fait qu'une clock
   se dérègle petit à petit, ça me parait déja dit; si c'est le fait que
   les lags réseaux provoquent un glissement, c'est dit aussi.}
...
of the end of a track with the beginning of the next one\ignore{\footnote{
...
the crossing operator needs twice as much stream data\ignore{\footnote{
...
\TODO{SM faudrait mentionner avant que c'est implémenté en OCaml}
