...
should be played).
Finally, tracks can contain data, which consists of interleaved sequences of samples 
...
Stream generation is performed top-down:
active sources actively attempt to produce their stream,
...
suivant}
\TODO{RB je trouve ça ok, un frame est une notion commune, et présenté
comme ça c'est suffisament simple. Par contre j'ai peur que pulling ici
donne l'intuition qu'on fait de la copie...}
...
This last ingredient is crucial to obtain the expected expressiveness.
For example, a rotation operator will play alternatively several sources,
but should only rotate at the end of tracks,
and its unused sources should not keep streaming -- otherwise we might find them
...
\TODO{DB J'ai pas parlé de cycles (clocks)
  ...
  temps...}
\TODO{RB bof, ici on parle de concepts généraux, ça me semble
suffisant..}
...
Having introduced all the terminology, we can give an example of 
...
call-by-reference mechanism based on frames, which represent a finite
amount of data. An active source instanciates a frame and tries to fill it.
To do this, it passes the frame to an underlying source, which either it fills the frame, 
possibly by passing it again to an underlying source and so on\dots
Using this method, streams can be generated using a finite amount of memory. The frames used
to generate streams are passed as memory pointers, which 
...
This description of a stream generator simple and natural. Unfortunately, it is too 
limited. For instance, one may want to have several outputs of the same 
stream, \eg\ one to a local file for archiving purposes and another one to an 
