...
\end{wrapfigure}
The continuous flow of multimedia (audio or video) data is modeled as a
\emph{stream}, which is a succession of \emph{tracks}. The tracks can be thought
as individual songs on a musical radio show. Concretely, these streams consist
of interleaved sequences of samples (for audio data), images (for video data),
etc. (see Section~\ref{sec:content}). Streams can also contain a very important
other kind of information: \emph{metadata}.\TODO{SM est-ce ça va de considérer
  les end-of-tracks comme des metadata particulières ??? (pour la théorie)}
These are punctual markers storing data, which can occur at any instant in the
stream, and are constituted in practice of a pair of strings storing the name of
the metadata and its value. They are used to store various information about the
stream such as track delimiters (the moment where a song ends and the next one
starts in a radio for example), copyright information (title, artist, etc. of
the current track), custom information about tracks (for example how loud it
should be played); see Figure~\ref{fig:stream}.
...
Being potentially infinite objects, streams are obviously not represented
directly and entirely but actually generated on the fly. They are produced
interactively by operators which are parametrized by various external factors
(variables possibly modified by external interfaces and metadata in particular).
...
operators on the stream in the sense that they produce a stream based on input
...
Some sources have a particular status: not only they compute a stream like any
other source, but they also perform some observable task, typically outputting its
stream somewhere. These sources are called \emph{active} sources.
\TODO{dire que les actives tirent le flux}
...
\TODO{DB en fait parler de frames est pe trop précis: on omet de dire
...
\TODO{DB faudrait se documenter mais je crois que c'est vraiment particulier:
...
The \liquidsoap{} language is interpreted. The evaluation of a script or
expression triggers the construction of a stream processing devices, built by
composing various elementary blocks.
...
\TODO{also mention labels to help users}

\subsection{The typing system}
\TODO{describe the typing system}

\subsection{Heterogeneous stream contents}
\label{sec:content}
...
\TODO{SM expliquer qu'on n'arrive pas à tout faire de façon statique}
