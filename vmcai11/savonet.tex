...
\[
\def\f#1{\save
[].[]!C="f#1"*+<20pt>[F--]\frm{}\ar @{--} "1,1"\restore}%
\def\g#1{\save
[ll].[].[d]!C="g#1"*+<20pt>[F--]\frm{}\ar @{--} "2,2"\restore}%
\xymatrix{
   \mathtt{clock_1} & *+[F]{\mathtt{input.alsa}}\ar[r]\f1&*+[F]{\mathtt{crossfade}}\ar[r]&  *+[F]{\mathtt{fallback}}\ar[r]&
  *+[F]{\mathtt{output.icecast}}\g1\\
   &\mathtt{clock_2} &  & *+[F]{\mathtt{playlist}}\ar[u]&
}
\]
\end{center}
...
When instantiating the graph, each operator declares if it needs a custom clock. Then, when initializing 
the graph of the stream generator, the clocks are attributed by a constraint satisfaction algorithm.
