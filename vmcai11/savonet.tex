...
other source, but they also perform some observable task, typically outputting their
...
obtaining data from their input sources
which in turn obtain data from their dependent sources, and so on.
...
\subsection{A language for building sources} % building streaming systems?

In addition to defining a streaming model, \liquidsoap\ provides the user
with a convenient high-level language for describing streaming systems.
Although our language borrows from other functional
programming languages, it is unique to our tool.
The main reasons for having created our own language are the need for
a static type discipline and a very user-friendly language,
tighly integrated with our tool.
