...
typing discipline together a very user-friendly language, tightly integrated
with our tool.
...
allowed us to integrate a few domain-specific extensions, and integrate it
tightly with the tool\TODO{SM on comprend pas trop ce que c'est que le tool, on
  peut pas juste dire que c'est l'interpréteur ou des libraries ?}, which offers
a browsable documentation of its scripting API and helpful error messages. In
practice, many of the users of \liquidsoap{} are motivated by creating a radio
and not very familiar with programming, so it can considered that the design of
...
its typing system, the reader interested into a detailed presentation referred
...
various operators built into the language. For example, the following script
defines two elementary sources, respectively reading from an HTTP stream and a
playlist of files, composed in a fallback (\ie{} if the HTTP stream is ended or
broken then the playlist is used), filtered through a volume normalizer and sent
to an Icecast server which will broadcast the stream to the listeners:%\TODO{DB define icecast}
\begin{verbatim}
...
to its frame to its underlying sources.
...
\subsection{Functional is useful: transitions}
...
\TODO{DB time/space/scope dictates that we don't describe the type system
  except for a few general sentences}

\subsection{Heterogeneous stream contents}
\label{sec:content}

Motivate why we need it (besides video and MIDI and good example
 is decoding a playlist of stereo AND mono audio)
and how we make sure it's consistent.

Fixed/variable arity, content kind, types and type constraints,
non-parametricity and type annotations, inference and encoding formats.

\subsection{Fallibility}

\TODO{SM expliquer qu'on n'arrive pas à tout faire de façon statique}
\TODO{DB Je le dis pour les clocks (qui est aussi une analyse dynamique
  faite au meme moment que la fallibilité).
  Sinon, je pense qu'on peut envisager
  de virer cette mini section si elle ne s'intègre pas naturellement
  ailleurs.}
