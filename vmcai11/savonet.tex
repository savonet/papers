...
\[
\def\f#1{\save
[ll].[]!C="f#1"*+<20pt>[F--]\frm{}\ar @{--} "1,1"\restore}%
\def\g#1{\save
[l].[]!C="g#1"*+<20pt>[F--]\frm{}\ar @{--} "2,1"\restore}%
\xymatrix{
   \mathtt{clock_1} &*+[F]{\mathtt{input.http}}\ar[r]&  *+[F]{\mathtt{fallback}}\ar[r]&
  *+[F]{\mathtt{output.icecast}}\f1\\
   \mathtt{clock_2} & *+[F]{\mathtt{playlist}}\ar[r] & *+[F]{\mathtt{crossfade}}\ar[u] \g1&&
}
\]
\end{center}
...
When instantiating the graph, each operator declares if it needs a custom clock. If an operator requires
a new clock, a new set is created and all the nodes below this operator are attached to the new clock.
Then, the attribution of clocks is done, as for filling frames, from the output to the sources.
Because of caching, we do not want a source to be connected to two different operators with 
