...
the meaning of \texttt{\%vorbis} is explained in Section~\ref{sec:typing-ex}.
...
TODO: not everything can be typed

 It also performs some sanity checks
such as checking that a source is \emph{infallible},
\TODO{SM je mettrais infaillible après, c'est chelou de mettre un détail avant le
  premier ex et en plus on dirait que \emph{toutes} les sources doivent toujours streamer}
\ie{} always has data available.
